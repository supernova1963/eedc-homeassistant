...
3. **Echter Einzelfall / bewusste Abweichung?** → **Maintainer-Freigabe**, doppelt dokumentiert: Code-Kommentar (warum + Referenz) **und** Ausnahmen-Liste (Startbestand: `EnergieFlussBackground.tsx` szenische Gradienten; CommunityShare Orange-Section-CTA; AktuellerMonat `text-5xl`-Hero-Zahlen; **R3b E4** (Gernot 2026-07-05): flexibler Ø-Netzbezugspreis BLAU hervorgehoben statt Strompreis-Purple — Hinweis „dynamischer Tarif aktiv", genau 2 Stellen: `v4/MonatRahmen.tsx` + `finanzen/TKonto.tsx`; **R3b E2**: WerteTabelle-`URTEIL_KLASSE` als eigenes 2-stufiges Delta-Urteil-Vokabular, bewusst ≠ AMPEL-Skala).
...
> **Rollen-Zwillinge (R3b S18, 2026-07-05 — Pointer, Werte in `lib/colors.ts`):**
> - **`TREND_TEXT_CLASS`** (positiv/negativ/neutral) für Trend-/Delta-Wertungen — der **Konsument bestimmt die Richtung** (Vorzeichen ≠ Wertung: steigende JAZ = positiv, steigender Verbrauch wäre negativ). Optisch = GELD-Grün/Rot, semantisch eigene Rolle.
> - **`CO2_TEXT_CLASS`** für CO₂-/Umwelt-Einsparungswerte (an `co2Pv`-Emerald verankert) — kein hartes `text-green-600` mehr.
> - **`STATUS_TEXT_CLASS`** — Text-Zwilling zu `STATUS_COLORS` (F3-Achse: ok grün · warnung GELB yellow-500 · kritisch rot · info blau); Status-Icons/-Legenden leiten daraus ab (Drift-Beleg: „warnung" stand als Solar-Amber in den Einstellungen).
...
> **Balken-Optik = eine globale Zentrale (G16-3, Gernot 2026-07-06, abgenommen).** Wo ein nativer Scrollbalken bewusst **bleibt** (Seiten-/Sicht-Scroller der Allowlist, vertikale Tabellen/Dropdowns/Modals ohne ScrollSchatten), trägt er den app-weiten Kanon-Stil aus **`index.css`** (die eine Zentrale, kein Inline-Style, kein JS): schmaler runder Pillen-Thumb (`::-webkit-scrollbar` 10 px, `border-radius:9999px`, `background-clip:padding-box` mit 2-px-Transparent-Rand = schwebend; Firefox `scrollbar-width:thin` + `scrollbar-color`), Thumb gray-300 / dark gray-600 auf **transparenter Spur** (Flächenfarbe scheint durch → dezente „Linie in Hintergrundfarbe"), Hover eine Stufe dunkler. Wirkt automatisch auf **jeden** Scroll-Container beider Achsen — ein SoT, keine Pro-Komponenten-Overrides. `.scrollbar-none` (Klassen-Spezifität) übersteuert weiter, wo ein Balken bewusst versteckt ist (u. a. in `ScrollSchatten` selbst). Die **Seiten-Scrollbar bleibt sichtbar** (Orientierung, D6-1). Ein echter fester „Punkt-auf-Linie"-Thumb bräuchte eine JS-Scrollbar-Lib → bewusst NICHT (Wartung/A11y/Perf); die Pille ist die pur-CSS-Näherung.
...
> - **Sortierung:** Typ-Spalten nach `INVESTITION_TYP_ORDER` (`lib/constants.ts` / `compareTyp`) als SoT; Suffix-Typen über Präfix-Match. **Datums-/Zeitreihen-Default absteigend (aktuell → alt), F10** — Ausnahmen (abschließend): Verlaufs-Charts (chronologisch) inkl. ihrer Tabellen-Ablesung `ChartDatenTabelle` (Paket CT: 1:1-Lesung des Charts) · **WerteTabelle-Zeitreihen-Default aufsteigend** (R3b E1, Gernot 2026-07-05: Lese-Richtung der Analyse-Tabelle, Kommentar an der Komponente) · **T-Konto-Monats-Select kalendarisch Jan→Dez** (R3b E5, Gernot 2026-07-05: Jahres-Kontext, Default-Auswahl bleibt der neueste Monat).
> - **Leerwert:** `—` aus A3.
...
> - **Achsen/Grid:** Farben aus `CHART_ACHSEN` über den Hook **`useChartTheme()`** (dark-aware, A8); Tick-Text via zentrale `index.css`-Overrides. Achsen-Beschriftung mit Einheit (C3), `fontSize: 11–12`. Zeitachse Slot-Konvention backward — **Slot `h` trägt `[h-1, h)`**, Beschriftung und Einsortierung ausschließlich über `lib/stundenSlot.ts` (s. u.).
...
> - **Legenden-Toggle = Standard bei Multi-Serie** (Gernot 2026-07-08, gebaut 2026-07-18): jeder Chart mit >1 gleichzeitig sichtbarer Serie (gestapelt ODER nebeneinander, auch Radar) macht die Legenden-Einträge klickbar — Klick/Enter blendet die Serie aus/ein (Recharts `hide`, Eintrag gedimmt, Y-Achse reskaliert auf Sichtbare; löst Größenordnungs-Lesbarkeit, z. B. PV ~20× BKW). **SoT = `hooks/useLegendenToggle`** + `ChartLegende.onItemClick` — keine lokalen `versteckt`-Set-Kopien. Zustand flüchtig (kein localStorage/C4), Reset bei Modus-/Preset-Wechsel (`resetSignal`). Key-Wahl chart-lokal: Default `dataKey`; Name-Key bei doppeltem dataKey (Finanzen Netto Bar+Trend); Paar-Mapping bei bidirektionalen `_pos`/`_neg`-Serien (Butterfly). **Kein** separates Zweit-Control für dieselbe Serie (Konfidenzband-Checkbox wurde dafür entfernt, Regel 0). **Ausnahmen (dokumentiert):** Werteklassen-Legenden (Cell-Färbung EINER Serie: Über/Unter Ø, Regionen, WP-vs-Gas) · `AnteilDonut` (Ausblenden verfälscht die %-Basis; Pie hat kein `hide`) · `WetterWidget` (Einzelfall D18-5) · Einzelserien.
> - **Chart-Daten-Tabelle = `ui/ChartDatenTabelle`** (Paket CT, Gernot 2026-07-18): die Tabellen-Ablesung eines Charts — 1 Zeile je X-Wert, 1 Spalte je Serie (Labels ≡ Legende/Tooltip, Header `Label (Einheit)` nach B2), de-DE-Zellformat, Summenzeile nur für summierbare Einheiten (kWh/km/€ — nie %, kW, ct/kWh), `CsvExportButton` (CSV = Rohwerte max. 4 NK). **Zugang NUR über den Chart-⇄-Tabelle-Umschalter in der `FokusVollbild`-Kopfzeile** (`Block.renderTabelle` bzw. `FokusKachel.tabelle` — kein Kartenkopf-Icon); **nie eine handgeschriebene Zweit-Tabelle je Chart** (Drift-Klasse). Kein Ersatz für kuratierte Sicht-Tabellen (`WerteTabelle` bleibt die reiche Analyse-Sicht). Zeilen in Chart-Reihenfolge = B2-Ausnahme „Verlaufs-Charts (chronologisch)". Piloten vor Flip: Cockpit-Monat-Verlauf · Jahr-Verlauf · Live-Tagesverlauf; Voll-Rollout nach Flip.
> - **Stunden-Zeitspanne = `lib/stundenSlot.ts`** (PN Rainer 90106, 2026-08-01): die Zeitachse ist **backward** (Slot `h` = `[h-1, h)`) — wer eine Stunde **beschriftet** (`slotZeitspanne`), eine Messreihe mit Slot-Beginn-Stempel **einsortiert** (`slotAusIntervallStart`) oder einen **Zeitpunkt-Marker** setzt (`slotAusZeitpunkt`, Sonnenauf-/-untergang), nimmt diese Stelle. Nie `` `${h}:00–${h+1}:00` `` daneben bauen: genau so trug Cockpit → Live „Wetter heute" eine andere Zeitspanne als Auswertungen → Prognose für **denselben** Punkt, und die IST-Kurve lag dort eine Spalte neben der Prognose. Client-Spiegel von `core/berechnungen/slot_konvention.py`; Regressionstests `lib/stundenSlot.test.ts` + `WetterWidget.test.tsx`. **Bewusst ohne baumweiten Wächter** — die Zuordnung ist an den drei bekannten Stellen gepinnt, eine *vierte* Sicht, die sich das Intervall wieder selbst baut, fängt heute kein Skript.
>   - **Unterhalb der Stunde gilt diese Regel NICHT — sie ist dort nicht entschieden** (Nebenfund **N-44**, 2026-08-02). Der Live-Butterfly beschriftet seine 10-Minuten-Punkte mit dem **Beginn** des Intervalls (`components/live/TagesverlaufChart.tsx:208`, „10:30 Uhr" = `[10:30, 10:40)`), der Block „Wetter heute" direkt darüber seit dem 01.08. mit dem **Ende**. Für sich ist beides vertretbar — ein Leistungspunkt darf seine Uhrzeit tragen —, nebeneinander gelesen ist es dieselbe Uneinigkeit eine Ebene feiner. **Wer das entscheidet, entscheidet es für alle Sub-Stunden-Raster** (Live-Kurve, Snapshot-Tabelle, Energieprofil-Tag, Fokus-Tabelle und die CSV-Spalte „Zeit") — deshalb nicht nebenbei in einem Bug-Auftrag. Bis dahin: **keine neue Sub-Stunden-Beschriftung erfinden**, sondern der jeweils benachbarten Sicht folgen und den Fund hier vermerken.
> - **Wächter:** `check:charts` (**statisch**, app-weit: R1 Pie-SoT-Pflicht · R2 keine rohe `<Legend>`) + `check:chart-audit` (**Laufzeit/Chromium**: Label-Overflow · Legende-Pflicht bei >1 *gerenderter* Serie — fängt `.map()`-Serien, die statisch unsichtbar sind · **L3 Toggle-Pflicht**: Multi-Serien-Legende muss klickbar sein, `role="button"` aus `ChartLegende.onItemClick`). Einzelserien (per-Instanz-gefärbt, z. B. WP-Saison) sind bewusst legende-frei.
...
> **Kontroll-Höhen-SoT (D14-5, 2026-07-03 · dritte Klasse R3b S5, 2026-07-05):** EINE Höhe je Kontroll-Klasse — **Formular-Kontrollen** (`ui/Input`, `ui/Select`, `ui/DatumFeld`, `.input` im Formular-Kontext) `min-h-[42px]`; **Buttons** `min-h-[36px]` im Button-Basis-Stil (die Größen `sm`/`md`/`lg` variieren nur Padding/Text, nie die Mindesthöhe); **Filter-/Toolbar-Leisten-Kontrollen** (Chips, Toggle-Pillen, Segment-Controls, Leisten-`<input>`/`<select>`) `h-8` = 32 px über die Konstante **`STEUER_H`** (`lib/komponentenStyle.ts`; detLAN #27 P2, Gernot 2026-06-28). Abgrenzung: 42 px gilt für Eingabefelder in Formularen/Wizards, 32 px für Bedien-Elemente einer kompakten Filter-/Toolbar-Zeile (dort brauchen `.input`-Felder `py-0`, Pillen `inline-flex items-center`). Keine lokalen `h-*`/`py-*`-Improvisationen an Kontrollen.
> **Segment-Umschalter = `ui/SegmentControl`** (R3b S4, 2026-07-05): zusammenhängende Optionsgruppe (Monat|Jahr, Erzeugung|Verbrauch, Diagramm|Tabelle, Kurzfristig|Langfristig …) über die SoT — Höhe `STEUER_H`, `aria-pressed` je Option, Aktiv-Stil gefülltes `bg-primary-600 text-white`. Der Primary-**Tint** (`bg-primary-100 …`) bleibt Navigations-Tabs vorbehalten (B3 „Aktiver Reiter"); scrollbare Pill-/Tab-Selektoren mit Badges (z. B. Komponenten-Geräteselektor, 44-px-Touch-Target) sind B3-Territorium, kein SegmentControl. Ein `value` ohne passende Option ist erlaubt (Shortcut-Paare wie Heute|Morgen: nichts aktiv).
> **Schalter = `ui/Switch`** (R3b S3, 2026-07-05; Seed c): EIN Toggle-Switch-SoT (`role="switch"`, h-6/w-11, Bahn `bg-primary-600`/`bg-gray-300 dark:bg-gray-600`). Dokumentierte Eigenschaft: der Knauf bleibt in beiden Modi `bg-white` (Kontrastträger auf der gefüllten Bahn — bewusst ohne `dark:`-Zwilling, A8-Kontrast-Ausnahme im Bauteil kommentiert).
> **Reload in v4-Sicht-Köpfen = `v4/ReloadButton`** (R3b S1, 2026-07-05): EIN geteilter „Aktualisieren"-Baustein auf `ui/Button` ghost+sm+`loading` — keine handgebauten Reload-Chips mit Hand-Spinner.
...
> **Der Dialog sagt Hilfsmitteln, dass er einer ist (2026-08-28):** `ui/Modal.tsx` trägt `role="dialog"`, `aria-modal="true"` und `aria-labelledby` auf seinen Titel — die `id` kommt aus `useId()`, nie aus einem eigenen Zähler: es stehen Dialoge gleichzeitig im Baum (Wizard-Modal + Rückfrage in `v4/EinstellungenModalHost`). Beim Öffnen bekommt der **Dialog selbst** den Fokus, nicht sein erstes Bedienelement — welches das ist, entscheidet der Inhalt, und in den Lösch-Dialogen wäre es die zerstörende Aktion. Beim Schließen geht der Fokus an das auslösende Element zurück, sofern es noch existiert. **Bewusst kein Fokusfang:** er wäre die einzige Zusicherung des Musters ohne Beleg — jsdom bewegt den Fokus bei `Tab` nicht (gemessen 28.08.), eine Vitest-Probe dafür wäre keine. Nachrüstbar; die Attribute sind seine Voraussetzung, nicht umgekehrt. Probe: `components/ui/Modal.test.tsx` (bis dahin hatte der SoT-Dialog von 28 Instanzen keine).

> **ESC hat einen Nachrang (2026-08-28):** Zuhörer auf `document` laufen in der Reihenfolge ihrer **Registrierung** — ein später geöffnetes Overlay läuft also NACH einem früheren, nicht davor. Wer die Taste verbraucht, meldet das mit `e.preventDefault()` (`ui/Modal`, `ui/DatumPicker`, `infothek/DateiLightbox`); wer darunter liegt, entscheidet **einen Makrotask später** (`setTimeout(…, 0)`) und lässt eine gemeldete Taste liegen (`blocks/FokusVollbild`). ⛔ **Kein `queueMicrotask`** — am 28.08. an der Box gemessen: Chrome fährt nach **jedem** Zuhörer einen Microtask-Checkpoint, ein Microtask sieht das `preventDefault` des nächsten Zuhörers also nicht; jsdom tut das nicht, die Vitest-Probe war grün, während die Anwendung Picker und Vollbild gemeinsam schloss. **Eine jsdom-Probe kann diese Reihenfolge nicht beweisen** — der Beleg ist der Lauf an der Dev-Box. **Ohne diese Regel nimmt ein ESC in Cockpit/Tag und /Monat den DatumPicker der Vollbild-Kopfzeile UND das Vollbild auf einmal.** Wer ein neues Overlay mit ESC baut, ordnet es hier ein.
>
> ⚑ **Die Regel gilt für jede verbrauchte Taste, nicht nur ESC** — und nur für die, die auch **wirkt**: `infothek/DateiLightbox` meldet ESC sowie ihre Blätter-Pfeile, am ersten bzw. letzten Bild aber **nicht** den dort wirkungslosen Pfeil; eine Taste zu sperren, ohne etwas zu tun, nimmt sie einem anderen Overlay umsonst. Die Lightbox war der vierte ESC-Verbraucher und meldete als einzige nichts. **Erreichbar war das nie** (sie lebt allein in `pages/InfothekTeile.tsx`, ohne `BlockShell`/`FokusKachel`, und kann mit dem Formular-Modal derselben Seite nicht gleichzeitig offen sein) — angeglichen wurde sie am 28.08. trotzdem, **statt es an „beim nächsten Anfassen der Infothek" zu hängen** (Gernot): *ein Trigger, der ein Ereignis braucht, das niemand plant, feuert nie.* Probe: `components/infothek/DateiLightbox.test.tsx` (bis dahin hatte die Fläche keine).

> **Fokus/Vollbild schließt mit ESC — einen Backdrop hat es nicht (2026-08-28):** `blocks/FokusVollbild` (SoT-Verweis oben im Block-Modell) ist deckend; es gibt kein Daneben, auf das man klicken könnte, die Backdrop-Hälfte der Konvention ist dort gegenstandslos. Es trägt bewusst **kein** `role="dialog"`: ohne Fokusverwaltung wäre `aria-modal` auf einem Portal-Overlay die Behauptung, es sei gelöst. Der Zuhörer hängt am Mount, nicht an einem `isOpen` — die Komponente ist entweder gerendert oder nicht da (Muster wie `infothek/DateiLightbox`).

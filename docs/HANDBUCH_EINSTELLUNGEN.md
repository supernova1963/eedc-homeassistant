...
- **Auto-Discovery:** eedc legt seine Entitäten in Home Assistant selbst an — du musst nichts von Hand eintragen. Alle gehören zu **einem** Gerät je Anlage; die Friendly Names tragen den Komponentennamen zur besseren Lesbarkeit. Seit diesem Release sind zwei Sorten dabei: Zahlen (`sensor.…`) und Schalter (`binary_sensor.…`, an/aus).
- **KPI-Export:** eedc exportiert Kennzahlen-Gruppen (Energie & Quoten, Finanzen & Investition, spezifischer Ertrag [aufs Jahr normiert], PV-Prognose, Börsenpreis-Trigger, **Steuerungshilfen**, **Sonstiges**). Die vollständige Liste mit Bedeutung und Einheiten steht in der **[Sensor-Referenz](SENSOR-REFERENZ.md)**.
- **Steuerungshilfen:** die Gruppe für Automationen — Überschuss (letzte Stunde und als Stundenreihe für heute), günstige Stunde, Speicher voll, Ladestand, Netzbezugs-Spitze, bestes Fenster für eine verschiebbare Last, Arbitrage-Vorschlag und eine Ampel für auffällige Prognose-Abweichungen. Je Gerät kommen Warmwasser-, Heiz- und Kühlfenster der Wärmepumpe dazu sowie Monatsverbrauch und Fenster eines sonstigen Verbrauchers. ⚠ **Es bleiben Vorschläge.** eedc schaltet nichts und ruft nichts auf; es nennt ein Fenster und einen Betrag — was daraus folgt, entscheidest du in deiner Automation.
- **Preise und Speicher (neu):** was **du** in der laufenden Stunde zahlst (`eedc_bezugspreis_jetzt_cent`), was du für eingespeisten Strom bekommst, was eine selbst verbrauchte Kilowattstunde damit spart, und was Strom aus dem Speicher kostet — je einmal für PV-Ladung und für Netzladung. Dazu „Speicher leer um", „reicht bis Mitternacht", der erwartete Abregelungsverlust an der Wechselrichter-Grenze und ein Schalter für negative Börsenpreise mit gleichzeitiger Erzeugung.
  - ⭐ **Der Preis kommt aus deinem Tarif, nicht von der Börse.** Bei einem Festpreis oder Zeitfenstern ist er exakt; bei einem dynamischen Tarif rechnet eedc `(1 + USt) × Börsenpreis + Aufschlag` und **leitet den Aufschlag selbst ab** — aus deiner letzten Abrechnung oder aus den gemessenen Stunden deines Preis-Sensors. **Du musst dafür nichts zusätzlich pflegen.** Jeder Sensor sagt im Attribut `preisquelle`/`aufschlag_quelle`, woher seine Zahl stammt.
  - ⚠ **Der Wirkungsgrad eines Speichers wird gemessen, nicht angenommen.** eedc rechnet ihn aus deinen erfassten Lade- und Entlademengen; nur wenn das nicht geht, nimmt es den gepflegten Parameter — und sagt im Attribut `wirkungsgrad_messung` warum. Gibt es **beides** nicht, entsteht der Sensor gar nicht: ein Vorgabewert wäre eine Behauptung über *dein* Gerät.
  - **Alle Stundenreihen tragen 24 Werte je Tag**, Index 0 ist die Stunde von 23:00 des Vortags bis 00:00. Jede Zeitangabe sagt, ob sie einen Beginn oder ein Ende meint — die vollständige Tabelle steht in der [Sensor-Referenz §11](SENSOR-REFERENZ.md).
...
>
> ⭐ **Seit diesem Release räumt er auch weg, was eedc früher einmal angelegt hat.** Bis März 2026 legte eedc je bequelltem Feld eine Entität `number.eedc_…_start` an, in die der Zählerstand vom Monatsanfang eingetragen wurde. Der Statistik-Import (§6.4) hat diesen Weg ersetzt — die alten Entitäten blieben in Home Assistant aber stehen und wurden von niemandem weggeräumt. Wenn du eedc vor März 2026 installiert hast, findest du sie unter `number.eedc_…_start`, alle mit dem Zustand „unbekannt". **„Sensoren entfernen" nimmt sie jetzt mit zurück**, und die Meldung sagt dir, wie viele es waren. Entitäten anderer Integrationen fasst eedc dabei nicht an — es räumt ausschließlich, was seinen eigenen Namen trägt.

**Startwerte:** *(Historisch — gilt nur für Installationen von vor März 2026.)* Der Monatswert entstand damals aus „aktueller Stand − Startwert", und der Startwert stand in einer `number.eedc_…_start`-Entität in Home Assistant. **Diesen Weg gibt es nicht mehr**: die Monatswerte kommen seit März 2026 aus der HA-Langzeitstatistik (§6.4), und es ist nichts mehr einzutragen. Die übrig gebliebenen `number`-Entitäten räumt „Sensoren entfernen" weg (s. Kasten darüber).

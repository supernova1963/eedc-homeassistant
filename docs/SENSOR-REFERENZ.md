...
| `wp_heizfenster_stunden` | h | Anzahl der Stunden innerhalb der erwarteten **Heizzeit**, in die sich das Heizprofil günstiger verschieben ließe. Attribute: `heizstrom_stundenprofil_kwh` (24 Slots, aus dem gelernten WP-Profil mit Heizgradtag-Korrektur), `guenstige_heizstunden`, `ersparnis_cent_vs_profil`, `heizzeit_stunden`. ⚠ **Was eedc nicht kennt und nicht erfindet:** Heizkurve, Vorlauftemperatur, Speichervermögen des Gebäudes. Wie weit eine Anhebung trägt, entscheidet deine Wärmepumpen-Regelung — eedc nennt Fenster und Preisvorteil, nicht die Gradzahl |
...
> **Wie die Kosten je Stunde entstehen.** Der Ausgangspunkt ist die **Börsenpreis-Stundenreihe** — dieselbe Zahl, die `eedc_preis_aktuell_cent` trägt (Attribut `preisquelle: boersenpreis`). In einer Stunde mit Überschuss sinkt der Preis anteilig: Wer dort zusätzlich verbraucht, verdrängt Einspeisung und keinen Netzbezug — bei 3 kWh Überschuss kostet die erste kWh nichts. Deshalb liegt ein Fenster mittags oft **vor** dem Preistal in der Nacht, obwohl der Börsenpreis dort höher ist. Liegt gar kein Preis vor, entstehen Fenster **nur innerhalb** der Überschuss-Stunden, und `preisquelle` sagt `keine`.
>
> ⚠ **Netzentgelte, Abgaben und dein Arbeitspreis stecken nicht darin.** Für die *Wahl* des Fensters und für das Delta gegenüber „jetzt" ändert ein fester Aufschlag nichts — er verschiebt jede Stunde um denselben Betrag. Für einen absoluten Eurobetrag rechnest du ihn in deiner Automation dazu.

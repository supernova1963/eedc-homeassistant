...
> ✅ **Die GELDSEITE von §9.2 ist am 06.09.2026 entschieden UND gebaut** (Bauschritt **11**, sechs
> Teilstücke): 11a Layer-SoT `investitions_jahresertrag.py` · 11b ROI-Dashboard · 11c Aussichten
> und HA-Export · 11d T-Konto-Beschriftung aus dem Backend · 11e Vorrang am Formular sichtbar ·
> 11f Doku. **Das Papier hat damit keinen offenen Bauschritt mehr**; die Release-Sperre 3
> („abgenommenes Konzept unvollständig gebaut") ist frei.
...
| ~~6~~ | ~~**Dauer-Anzeige schreibt ihre Annahme aus**~~ | ~~klein~~ | ✅ **gebaut 2026-08-10.** Formuliert wird der Satz **einmal** (`core/berechnungen/kapitalrechnung.py::annahme_dauer_text`) und von jedem Ausgabeweg abgeholt. ⚑ **„Die Dauer-Anzeige" gibt es nicht — es sind neun Stellen in fünf Dateien**, gespeist aus vier Quellen: ROI-Dashboard gesamt (Kachel · Break-Even-Kurve · Summenzeile · v4-Block-Zusammenfassung) und je Zeile (Tabellen-Tooltip), PDF-Finanzbericht, HA-Sensor `amortisation_jahre` — dort **im Rechenweg-Attribut**, weil ein Sensor keinen Tooltip hat. ⛔ **Hier stand bis 2026-08-29:** „Dazu die Wallbox-Dauer im Komponenten-Hub, die im Client aus `Anschaffung ÷ Ersparnis` entsteht und deshalb den festen Modell-A-Text aus `lib/amortisationAnnahme.ts` trägt.“ Genau diese Ausnahme war der Fehler (**N-230**): Der Client teilte durch die **rohen Anschaffungskosten** statt durch den **Kapitaleinsatz** — ohne Alternativkosten, ohne Förderung, ohne Betriebskosten-Abzug. Eine geförderte Wallbox bekam damit eine zu lange Dauer, direkt neben der Zahl des ROI-Dashboards. **Seit 2026-08-29 ist der Wallbox-Hub eine fünfte Backend-Quelle** (`dashboards.py::get_wallbox_dashboard` liefert `amortisation_jahre` + `amortisation_annahme`); der Client zeigt nur noch an, die Modell-A-Konstante ist ersatzlos entfernt. ⭐ **Die damals offene Zähler-Frage ist am 2026-09-01 entschieden (Gernot) — nicht neu aufrollen.** Der Wallbox-Hub behält seine gemessene Heimlade-Ersparnis, denn er beantwortet eine **isolierte** Frage („was war diese Box gegenüber öffentlichem Laden wert?"). In die **Kapitalrechnung der Anlage** kommt sie **nicht**: Die Ersparnis gehört der **E-Mobilität als Ganzes** und steckt bereits in der E-Auto-Zeile — deren Formel rechnet `Benzinkosten − E-Auto-Netzstromkosten` und unterstellt damit schon, dass **zuhause** geladen wurde (`aussichten.py`, `jahres_eauto_km_ersparnis`; der Kommentar dort sagt es wörtlich: „PV-Ladung ist in EV-Ersparnis"). Ein zweiter Posten daneben hätte dieselbe Kilowattstunde gegen **zwei einander ausschließende** Alternativen gerechnet — in der Benzin-Welt wird gar nicht geladen, in der Säulen-Welt kein Benzin gekauft. Das ist die Klasse aus **v4.0.20** (55,9 ct für dieselbe kWh). ⚑ Gernots Begründung im Wortlaut: *„Es war mein Verständnisfehler, zwanghaft eine Ersparnis der Wallbox zu fordern, die sie nicht hat. Wir fassen es ja als E-Mobilität sowieso zusammen."* **Die ROI-Zeile der Wallbox trägt deshalb keinen eigenen Zähler** — sie steht seit 2026-09-01 ehrlich auf **„nicht bewertet"**, statt eine 0 zu behaupten (N-351). ⚠ **Und die Zeile kannte den zweiten Fall nicht:** sobald `betriebskosten_jahr` gepflegt ist, rechnet eedc **Modell C** — der Betrag ist im Zähler bereits abgezogen, „ohne künftige Instandhaltung" wäre dort eine falsche Aussage über die eigene Rechnung. Der Text richtet sich deshalb nach den Daten. **Am Dev-Bestand gemessen:** die Anlage trägt 500 €/Jahr ⇒ „inkl. 500,00 €/Jahr Betriebskosten …", die WP-Zeile 200 €, das BHKW 300 €, alle übrigen Zeilen Modell A — der statische Satz wäre dort in **drei** von vier Sichten falsch gewesen. **Keine Zahl bewegt sich**, nur ihre Beschriftung |
...
| ~~11~~ | ~~**Geldseite der Abgabe an Dritte** (§9.2 Geldseite)~~ | ~~mittel–groß~~ | ✅ **gebaut 2026-09-06** in sechs Teilstücken: **11a** Layer-SoT `core/berechnungen/investitions_jahresertrag.py` (`jahresertrag_posten` — eine Antwort statt dreier Summen) · **11b** ROI-Dashboard samt kategorie-bewusstem „nicht bewertet"-Hinweis · **11c** Aussichten-Prognose und HA-Export · **11d** T-Konto-Beschriftung aus dem Backend (`erloes_label`; der eine Anzeigename steht in `field_definitions.SONSTIGES_ABGABE_LABEL`, aus ihm ist `BEZEICHNUNG_ABGABE` abgeleitet) · **11e** Vorrang *gemessen vor geschätzt* am Feld „Ertrag/Jahr (€)" sichtbar (E3-Auflage) · **11f** Handbuch, Glossar, Berechnungen, Style-Guide. Proben `test_investitions_jahresertrag_sot.py`, `test_abgabe_geldseite_drei_sichten.py`, `test_abgabe_geldseite_beschriftung.py`, `components/finanzen/TKonto.test.tsx`, `components/forms/InvestitionForm.test.tsx`. ⭐ **Die beim Bau zu messende Frage ist beantwortet — die Aussichten-Zerlegung (§8/7) behandelt Abgabe-Geräte wie Erzeuger, und das ist richtig:** der Zuordnungs-Block liest jedes *Sonstiges*-Gerät mit gepflegtem `einspeise_erloes_euro` **kategorie-blind** und ordnet direkt zu (Bauschritt-5-Regel „alles komponentenscharf Vorliegende direkt"). Doppelt zählen kann er nicht, weil `zerlege_kumulierten_ertrag` **verteilt** statt zu addieren; beides hält jetzt eine Probe fest, statt es anzunehmen. ⚠ **CHANGELOG-pflichtig** — ROI und Amortisationsdauer bewegen sich bei jeder Anlage mit Abgabe-Gerät und gepflegtem Erlös sichtbar (besser). ⛔ **KEINE Daten-Checker-Kategorie** gebaut (Tor 3, s. §9.2) |
...
> ✅ **Die GELDSEITE ist am 06.09.2026 entschieden UND GEBAUT** (Bauschritt 11, sechs Teilstücke) —
...
rilmor-mhrs nach der Umstellung auf die Kategorie).** ✅ **Status: ENTSCHIEDEN UND GEBAUT
(2026-09-06)** — Bauschritt 11 der Liste in §8, dort die sechs Teilstücke samt Proben.

...
> **ARCHIVIERT (2026-05-19)** — 3c-Kern in **v3.26.8** ausgeliefert, weitere Befunde durch Etappe 3d (v3.27.0), Etappe 4+5 (v3.31.0) und Etappe 6 (v3.31.1) abgearbeitet. Re-Audit-Resultat: Befund 1 (Counter Forward statt Backward) ✅ Etappe 3c P2 in `eedc/backend/services/snapshot/aggregator.py:530`. Befund 2 (Tagesgesamt-Wege gemischt): kWh-Anteil ✅ Etappe 4, Counter-Anteil als Spin-off festgehalten in [`KONZEPT-COUNTER-DAILY-DRIFT.md`](../KONZEPT-COUNTER-DAILY-DRIFT.md) (niedrige Priorität, anlassbezogen). Befund 3 (`quelle`-Spalte auf `sensor_snapshots`) ✅ `eedc/backend/models/sensor_snapshot.py:35`. Befund 4a (Backend Resnap/Reaggregat-Trennung) ✅ v3.26.6. Befund 4b (UI-Trennung explizit, `applyMode: with_resnap | rebuild_only`) ✅. Befund 4c (AbortController) ✅. Befund 4d (Modal-X im Apply-Zustand) ✅. Sektion 7 Modul-Refactoring ✅ Sub-Pakete `snapshot/` + `energie_profil/` existieren.

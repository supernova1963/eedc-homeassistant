...
> **ARCHIVIERT (2026-08-13)** — vollständig abgearbeitet. Phase 1 (v4.0.9), Phase 2 und Phase 3 (v4.0.14) sind ausgeliefert, **Phase 4 ist gestrichen** (Begründung in §Phase 4: die Substanz ist gebaut, die Bedingung zeigte auf zwei Anker, die es nicht gibt), und alle vier §Offenen Fragen sind erledigt — 1 + 2 durch #264, 3 verworfen (Gernot), 4 gebaut (N-239, kapazitätsgewichteter Anlagen-SoC). Die aus Phase 4 hervorgegangene Restfrage („Speicher voll um" rechnete mit dem Typenschild, N-238) ist mit v4.0.14 ebenfalls gebaut: gerechnet wird jetzt mit dem gepflegten Wirkungsgrad. **[#358](https://github.com/supernova1963/eedc-homeassistant/issues/358) ist am 2026-08-13 geschlossen.** Lebende Quellen für den aktuellen Stand: [CHANGELOG](../../CHANGELOG.md), Roadmap-Issue [#110](https://github.com/supernova1963/eedc-homeassistant/issues/110) und die Layer-Docstrings (`core/berechnungen/speicher_potential.py` · `speicher_sizing.py` · `speicher.py`) sowie [BERECHNUNGEN §3.3](../BERECHNUNGEN.md). Alles unterhalb ist der Stand vom 2026-08-12 und wird nicht mehr fortgeschrieben.

> **Status (gemessen 2026-08-12): Phase 1 + Phase 2 + Phase 3 ausgeliefert · Phase 4 offen, getrackt als [#358](https://github.com/supernova1963/eedc-homeassistant/issues/358).** ⚠ **Diese Zeile ist mit dem Archiv-Kopf darüber überholt** — sie stand hier noch auf „Phase 4 offen", während der Kasten drei Absätze tiefer die Streichung selbst beschreibt. Genau der Fehler, den sie im nächsten Satz benennt; er ist ihr ein zweites Mal passiert. ⚠ Bis 2026-08-12 nannte diese Zeile „Kern weiterhin offen" und verwies auf **#243**, das mit v4.0.0 geschlossen wurde — während drei Absätze tiefer im selben Kasten „Phase 1 ausgeliefert" steht. Der **Statuskopf** ist die Stelle, die jeder zuerst liest und die beim Fortschreiben des Rumpfes niemand anfasst (Fund N-182). Alles unter dieser Zeile bleibt inhaltlich gültig. Einzel-Issue #142 (rapahl) wurde 2026-05-23 in die Roadmap [#110](https://github.com/supernova1963/eedc-homeassistant/issues/110) verschoben. **Ort-Entscheid (IA-V4):** die Tiefe kommt als **Ausbau des Komponenten-Hubs Speicher** (SPEC-KOMPONENTEN K-O2), NICHT als eigener Auswertungen-Tab — Phase-1-Verortung unten entsprechend re-lesen. **⚑ Präzisiert 2026-08-01 (Gernot, nach Community-Einwand unter #350):** es gilt die **Ortsregel nach Zeitraum** — **zeitbezogene** Sichten (Tag/Monat/Jahr, also Phase 1: Monats-Tabelle, KPI-Kacheln, Sommer/Winter) gehören ins **Cockpit** neben die Energiebilanz; im **Komponenten-Hub** bleibt, was über die **Lebensdauer** des Geräts geht (Phase 2 SoC-Heatmap, Phase 3 Sizing-Simulator, Wirtschaftlichkeit). „Nicht als eigener Auswertungen-Tab" bleibt unverändert gültig — die Korrektur betrifft Hub ↔ Cockpit, nicht den Auswertungen-Bereich. **Seit Konzept-Erstellung bereits geliefert (vor B11-Bau einarbeiten):**
...
4. **Multi-Speicher-Anlagen** — ⚑ **aus diesem Konzept herausgelöst (Gernot, 2026-08-12): eigenes Issue.** Seine Linie: *„Das SoC anlagenweit betrachtet wird ist generell korrekt. Es muss nur sichergestellt werden, dass die Information über den SoC jedes Speichers sichtbar ist."*
   ...
   ✅ **GEBAUT (2026-08-12) — und dabei fiel ein Defekt auf, der größer war als die Frage (N-239).** Der gespeicherte `TagesEnergieProfil.soc_prozent` war bei mehreren Speichern **kein anlagenweiter Mischwert**, wie dieses Konzept und beide Phasen-Sichten behaupteten: `energie_profil/_helpers.py::_get_soc_history` nahm den **ersten** gemappten SoC-Sensor mit Daten und brach ab (`return` im LTS-Pfad, `break  # Erstes SoC-Entity reicht` im History-Pfad). Betroffen war alles darauf: Vollzyklen des Tages, SoC-Hübe, Phase-2-Heatmap, Phase-3-Kalibrierung.

   **Vollständig behoben statt richtiggestellt** (Gernots Entscheid: *„Das kann nur eine Notlösung sein, weil wir keine Lust haben, es richtig zu machen"*):

   - `_get_soc_history` liest **jeden** gemappten Speicher-Sensor und liefert `{stunde: {investition_id: soc}}`.
   - Neue Layer-Formel `core/berechnungen/speicher.py::anlagen_soc_prozent` bildet den Anlagenwert **kapazitätsgewichtet** (Σ Inhalt ÷ Σ Kapazität). ⚑ **Nicht** arithmetisch: 15 kWh auf 20 % und 5 kWh auf 100 % sind **40 %**, das ungewichtete Mittel behauptete 60 % — anderthalb Mal so viel Energie, wie im Haus steht.
   - Neue Spalte `TagesEnergieProfil.soc_je_speicher` trägt die Aufschlüsselung. Bewusst **eigene** Spalte und nicht das `komponenten`-Dict: das trägt kW/kWh und wird von Whitelist-Konsumenten summiert — ein Prozentwert darin wäre genau die Einheiten-Verwechslung, aus der der BKW-Doppelzählungs-Bug entstand.
   - **Kein Backfill** (der ausgeschlossene „große Heiler-Knopf"). Alte Tage behalten die Ein-Gerät-Zahl; der neue Daten-Checker-Befund **`SOC_NUR_EIN_SPEICHER`** meldet sie und stellt die Neu-Aggregation daneben. Erkennung ohne HA-Read und ohne Schwelle: ≥ 2 Speicher mit SoC-Sensor **und** `soc_je_speicher IS NULL`.

   ⚑ **Die Zusicherung, die das vor einem Release trägt:** bei **einem** Speicher ist das gewichtete Mittel über ein Gerät exakt dessen SoC — für die überwiegende Mehrheit der Anlagen ist die Umstellung beweisbar ein **No-op**, und genau das prüft der erste Test der Datei `test_soc_je_speicher.py` über den ganzen Wertebereich.

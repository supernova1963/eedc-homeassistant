...
> **✅ Entschieden (2026-06-13, Gernot — nach E1-P3-Vorschau): Universelles Block-Modell — JEDER Block einklappbar + Fokus/Vollbild, app-weit.** Nicht nur die Detail-Sektionen, sondern **alle** Blöcke einer Inhaltsseite — KPI-Strip, Hauptblock, Werte/Tabelle-Sektion **und** Detail-Sektionen — sind (a) **einklappbar** (Zustand pro Sicht persistiert, B6) und (b) tragen einen **Fokus/Vollbild-Schalter**, der den Block bildschirmfüllend macht und die übrigen ausblendet. Das gilt **app-weit auf allen Inhalts-Achsen**: Cockpit, Komponenten, Auswertungen (und Community). **Anlass:** Ohne diesen Schalter verlöre die beliebte **Live-Seite** ihren bildschirmfüllenden Energiefluss, sobald sie ins universelle Muster einzieht — der Fokus-Modus erhält genau das (Live startet Fluss-dominant, ⤢ macht ihn voll). Nützt ebenso großen Charts/Tabellen, besonders auf Mobile. **Reorder (↑↓)** bleibt den **Cockpit-Zeitsichten** vorbehalten (persönliche Reihenfolge des festen Satzes); Komponenten/Auswertungen behalten ihre **feste** Reihenfolge (Stabilität/analytische Logik), sind dort aber ebenso einklapp- und fokussierbar. **Weiterhin kein Widget-Builder** (Einstellbarkeits-Cap) — es werden nur feste Blöcke geklappt/fokussiert/ (in Cockpit) umsortiert, keine frei wählbaren Bausteine. Fokus ist ein transienter Sicht-Modus (nicht persistiert), Klapp-Zustand wird persistiert.
>
> **Default-Klappregel (2026-06-19, Gernot — Cockpit/Monat-Referenz; revidiert):** Im *Erstzustand* einer Sicht (bevor der Nutzer eigene Klapp-Zustände gesetzt hat) ist nur der **erste Block** offen — in Cockpit/Monat die **Kennzahlen** (KPI-Strip). Alle übrigen Blöcke (Energie-Bilanz, Verlauf, Komponenten-Sektionen, Finanzen) starten **eingeklappt**; ihre **Summary-Zeile** trägt den Kern, sodass der Überblick scanbar bleibt und Details gezielt geöffnet werden. (Die Kennzahlen-Kachel hat eingeklappt keine sinnvolle Summary — sie *ist* die Zusammenfassung — und bleibt darum offen.) Sobald ein Nutzer Klapp-/Reihenfolge-Zustände gesetzt hat, gilt seine persistierte Wahl (B6); ein **Reset-Knopf** („↺ zurücksetzen", erscheint nur bei Abweichung) stellt den Default wieder her.
...
> **Umsetzungs-Entscheid A.3 (2026-06-22, gebaut `CockpitLiveV4`, überstimmt obigen Absatz):** „so nah wie möglich am IST" — **kein Linsen-Toggle** (Energiefluss bleibt fest, **Tagesverlauf** ist eigener Vollbreiten-Block direkt darunter, wie IST) und **keine** sortierbare Block-Stapelung; die Nebensektionen liegen wie im IST **als Sidebar unter „Heute"**. Neu **durchgängig**: ein **Fokus/Vollbild-Schalter (⤢) je Karte** (geteilte SoT `FokusVollbild`/`FokusKachel`, BlockShell konsumiert dasselbe Overlay). Die finale Konsolidierung dieses Absatzes erfolgt am **Konzept-Gate** (nach dem Muster-Satz).
...
| I9 | **Navigations-Invariante:** V4 verlinkt NIE nach V3 — kein `'#/…'`-Hash-Literal ≠ `#/v4/` unter `src/v4/**`; geteilte `*Teile` nutzen `useV4Basis`, re-kategorisierte Ziele `v3RouteZuV4`. (Rest bis R6 auf **4 `!istV4`-gated V3-Fallback-Literale** in 2 Dateien reduziert — MonatsdatenTeile 3× + ctx.navigate-Adapter; die früheren „Donor-Ziele ohne V4-Heimat" sind aufgelöst, s. Restweg §3. Am Flip bleibt nur die Donor-LÖSCHUNG, keine Link-Arbeit.) | `hooks/useV4Basis.ts` · `config/v3ZuV4Route.ts` | `check:v4links` · `check:v4-migration` (navigate→V3-Zähler) |
...
| I12 | **SoT-Komponenten-Pflicht (Regel 0/0a):** eine Komponenten-Klasse = EINE Komponente (KPICard, Button, CsvExportButton, Modal, ChartTooltip/-Legende, Table/WerteTabelle, KpiStrip, **SegmentControl, Switch, ReloadButton, InlineAktion, ErfassungZustandBadge, AnteilDonut, KomponentenFinanzTabelle** …). Die Liste wächst mit der Zentrale (`components/ui/index.ts`) — sie ist dort ablesbar, nicht in einem zweiten Register. Neues erweitert die Zentrale — nie ein Zweitbau fürs bestehende Pattern. | Style-Guide Regel 0/0a + Teil B · SoT-Inventar | Review + das **Wächter-Register** (25 `check:*`, `package.json` — s. Register-Notiz unter der Tabelle, Stand 2026-07-20) |
| I13 | **Tabellen-SoT (Regel T):** Tabellen rendern über die Table-SoT (`ui/Table`/`WerteTabelle`; `ChartDatenTabelle` für die Chart-Ablesung) — Spalten-Header Satz-Stil + Einheit in runden Klammern `Name (Einheit)`, Zahlen rechtsbündig de-DE, Summenzeile nur für summierbare Einheiten (kWh/km/€), Sortier-Konvention (Typ = `INVESTITION_TYP_ORDER`/`compareTyp`, Datumsreihen absteigend; dokumentierte Ausnahmen). | Style-Guide **B2 + Regel T** · [`KONZEPT-TABELLEN-SOT`](drafts/archive/KONZEPT-TABELLEN-SOT.md) (Abnahme 2026-07-10) | `check:tabellen` |
| I14 | **Chart-SoT (B7):** Charts nutzen die Chart-Bausteine — Serienfarben aus `lib/colors.ts`, `ChartTooltip`/`ChartLegende` (Swatch + monochromer Text), `AnteilDonut` für Anteile, `useChartTheme`-Achsen; **Multi-Serien-Charts haben klickbare Legenden-Toggle** (`hooks/useLegendenToggle` + `ChartLegende.onItemClick`, flüchtig). Keine rohe `<Legend>`, keine Hand-Farbkästchen. | Style-Guide **B7** · `hooks/useLegendenToggle` | `check:charts` · `check:chart-audit` (inkl. **L3** Toggle-Pflicht) · `check:chart-tooltip` |
...
- **Theme-Editor / freie Card-/Widget-Anordnung (Dashboard-Builder, „My-Sites") / Dichte-Profile** — siehe Style-Guide „Einstellbarkeits-Cap" (Hell/Dunkel-Mode + Mobile-Reduce-Default sind die einzigen Achsen). **Klarstellung (2026-06-01, erweitert 2026-06-13):** Das *Einklappen* jedes Blocks, der *Fokus/Vollbild-Modus* (universelles Block-Modell) und das *Umsortieren des festen Sektionssatzes* in den Cockpit-Zeitsichten sind davon ausgenommen und ausdrücklich erlaubt — gearbeitet wird immer nur am *festen* Block-/Sektionssatz, es ist kein Builder.
...
| Beliebte Live-Seite verliert bildschirmfüllenden Energiefluss im universellen Muster | **Universelles Block-Modell mit Fokus/Vollbild-Schalter** (Entscheid 2026-06-13): jeder Block — auch der Live-Energiefluss — lässt sich auf Vollbild schalten, übrige Blöcke einklappen; Live startet Fluss-dominant. In der E1-P3-Vorschau demonstriert |

...
>
> **Noch selbst faltend:** `cockpit/uebersicht.py` + `ha_export.py` (S4), `investitionen/dashboards.py` (S5), `services/community_service.py` (S6) sowie die beiden Performance-Ratio-Pfade in `aussichten.py` (Langfrist/Trend — gleiche Klasse, in der Inventur nicht erfasst). **`aussichten.py` ist damit teilmigriert:** die Monatsgrößen der Finanz-Prognose kommen aus der Schicht, die per-Investition-Aggregate (WP-Alternativkosten, E-Auto je Fahrzeug, dienstliche Ladekosten) faltet sie weiter selbst.

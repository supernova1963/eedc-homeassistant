...
>
> **Was offen bleibt (N-107):** `get_aktueller_monat` faltet weiter anlagenweit über eine **eigene** IMD-Batch — der Komponenten-Detailblock (Speicher-Netzladung + Ø-Ladepreis, WP-Heiz-/WW-Split, eMob Netz/Extern/V2H, BKW-Eigenverbrauch, die sechs Sonstiges-Summen). Vier dieser Größen hat die Schicht heute nicht, zwei änderten beim Umhängen ihre Bedeutung. Der C1-Auftrag hatte diesen Select als rein per-Investition eingeordnet; am Code gemessen ist er es nicht — deshalb steht die Restschuld bei **1** und nicht bei 0. Umbuchen ohne Migration wäre Umetikettieren.
>
> | Kategorie | Bedeutung | Stand nach C1c |
> | --- | --- | --- |
> | `P10_SCHREIBEN_IMPORT_CHECKER` | schreibt, prüft oder reicht durch — leitet nichts ab; auf Dauer legitim | 15 Funktionen |
> | `P10_PER_INVESTITION` | Aggregat **je Gerät**; die Schicht hat dafür keine Sicht (Register **N-2**) | 12 Funktionen |
> | `P10_NOCH_NICHT_MIGRIERT` | faltet eine **anlagenweite** Monatszeile selbst — die Klasse, die P10 schließen soll | **1**, mit Obergrenze im Test |
>
> **Offene Schuld, benannt statt versteckt:** `aktueller_monat.py` (Monatsbericht + Vorjahr, **N-16** — zwei Funktionen). Sie stand wie N-15 und N-17 nie im Bauplan und ist beim Scharfstellen aufgefallen; die beiden anderen sind mit **C1a** und **C1b** getilgt. `test_p10_offene_schuld_waechst_nicht` hält die Zahl bei **2** (vorher 3 nach C1a, 4 nach S6, 5 nach S5) — sie darf nur sinken; eine Obergrenze, die nach einem erledigten Schritt stehen bleibt, ist keine. **Teilmigriert** bleiben `aussichten.py` und `ha_export.py`: ihre Monatsgrößen kommen aus der Schicht, die per-Investition-Aggregate (WP-Alternativkosten, E-Auto je Fahrzeug, `calculate_investition_sensors`) falten sie weiter selbst.

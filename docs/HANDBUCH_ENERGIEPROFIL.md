...
- **Kumulative kWh-Zähler, keine Leistungssensoren.** Der Aggregator liest ausschließlich fortlaufende kWh-Zählerstände. Ein **Leistungssensor (W/kW)** in einem kWh-Feld liefert physikalisch unmögliche Stundenwerte. Basis der Anlage sind `einspeisung` **und** `netzbezug` — fehlt einer, bleibt der bilanziell gerechnete Gesamtverbrauch leer. **Hat die Anlage einen Speicher, gehören seine beiden Zähler (Ladung und Entladung) genauso dazu:** Der Verbrauch ist eine Differenz aus vier Größen, und ohne die Batterie wäre er nachts nur der Netzbezug — also genau dann zu klein, wenn der Speicher liefert. Fehlt eine der beiden Richtungen, bleibt die Stunde deshalb leer und der Daten-Checker sagt es (siehe [Daten-Checker §4.25](HANDBUCH_DATEN_CHECKER.md#425-speicher-zaehlerstaende-unvollstaendig)).

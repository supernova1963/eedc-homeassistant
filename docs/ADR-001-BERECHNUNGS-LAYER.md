...
### Nachtrag 2026-08-04 (N-137): eine Kennzahl ohne Anzeige driftet unbemerkt weiter

Dritter Fall, und der lehrreichste: Die Frage „was hat die Anlage relevant gekostet?" hatte **drei** Antworten im Baum. Die USt-Bemessung war seit N-129/N-130 auf `Σ max(0, gesamt − alternativ)` festgelegt; die ROI-Sicht rechnete `Σ (gesamt − alternativ)` **ohne Klemmung je Position**; und `aussichten.py` wie `cockpit/uebersicht.py` trugen — wortgleich, also eine Kopie — eine **Hybrid-Summe** `PV-System voll + WP-/eAuto-Mehrkosten + Sonstiges voll`, deren Mehrkosten aus `inv.parameter["alternativ_kosten_euro"]` kamen. Dieser Schlüssel hat **baumweit keinen Schreiber**: kein Formular, kein Wizard, kein Import setzt ihn. Gepflegt wird die Spalte `anschaffungskosten_alternativ` — die der **Daten-Checker mit WARNING einfordert** („werden für ROI-Berechnung benötigt"). Zwei von drei Sichten rechneten also mit Festannahmen (8.000 € / 35.000 €) an genau dem Feld vorbei, nach dem eedc den Anwender fragt.

**Warum es niemandem auffiel:** Die Hybrid-Summe erreichte keine Oberfläche. Ihre Konsumenten waren der Amortisations-Fortschritt der Aussichten und der ROI-Fortschritt des Cockpits — der eine hatte beim V4-Flip seine Anzeige verloren, der andere wird nicht gerendert. Am **Demo-Datenbestand** stimmen beide Summen zufällig überein (76.500 €), weil der Seed die Alternativkosten exakt auf die Festannahmen setzt; die Differenz entsteht erst bei einer gepflegten Alternative ≠ 8.000/35.000.

**Die Lehre:**

> **Eine ausgelieferte, aber unangezeigte Kennzahl ist kein toter Code — sie ist eine Drift, die auf ihren Auftritt wartet.** Sobald jemand die Anzeige (wieder) einbaut, wird die falsche Zahl sichtbar, und zwar als Regression *dieser* Änderung. Wer eine solche Größe findet, hat zwei ehrliche Möglichkeiten: sie auf den SoT ziehen oder sie entfernen. Sie stehenzulassen verschiebt die Kosten auf den nächsten, der die Fläche anfasst.

...
- **Monatsabschluss-Wizard: Schreib-Reject-Diagnose** (Vorbereitung für #251 FrodoVDR-Folge): Bisher wurde der `WriteResult.applied`-Status der Provenance-Resolver-Aufrufe im Wizard nicht ausgewertet — User sah „erfolgreich gespeichert", auch wenn der Resolver intern abgelehnt hatte. Tritt in der Praxis nur in seltenen Konstellationen auf (z. B. wenn die existing-Provenance bereits `repair` ist, was eine niedrigere Manual-Eingabe blockiert). Jetzt: pro abgelehntes Feld erscheint eine Warnung mit Klartext (Feldname + konfliktierende Quelle) im Wizard. Defensive Maßnahme — der ursprünglich von FrodoVDR berichtete Bug („Werte nicht eintragbar") liegt vermutlich woanders, Diagnose läuft.

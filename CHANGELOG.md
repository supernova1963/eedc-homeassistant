...
- **Drift-Inventur Tier-1 (Paket DI) — bekannte Kennzahl-Abweichungen vor dem großen Release bereinigt.** Einzelne Kennzahlen wurden an verschiedenen Stellen leicht unterschiedlich gerechnet. Korrektur, kein Rechenfehler in der Kern-Bilanz — aber betroffene Werte ändern sich sichtbar:
  ...
  - **DI-2 — HA-Export-Sensor „CO2 Einsparung" umfasst jetzt die volle CO₂-Bilanz.** Der nach Home Assistant exportierte Sensor rechnete bisher nur die PV-Erzeugung (`PV × 0,38`) — Wärmepumpe und E-Mobilität fehlten ganz. Jetzt trägt er die **gleiche Gesamt-Bilanz wie die Cockpit-Kachel** (PV-Eigenverbrauch + WP + E-Mobilität, Dienstwagen ausgeschlossen), berechnet über den kanonischen Helper `berechne_co2_bilanz`. Demo (Anlage 1, gesamt): **12.538 → 11.226 kg**. Hinweis: Da es ein `total_increasing`-Sensor ist, macht die HA-Langzeitstatistik einmalig einen Sprung. (Die dabei bemerkte Rest-Abweichung bei Balkonkraftwerk-Anlagen ist mit **DI-2-B** behoben, siehe unten.)
  - **DI-3 — Dienstwagen zählten im Jahres-/Anlagenbericht (PDF) fälschlich als private E-Mobilität.** Die E-Mob-Aggregation des Berichts summierte gefahrene Kilometer, Heimladung und V2H über **alle** E-Autos/Wallboxen — auch dienstliche. Dadurch waren km, CO₂-Ersparnis und Heimladungs-Kennzahlen zu hoch (die Ladung eines Dienstwagens ist eine Ausgabe, kein Ertrag). Cockpit, Komponenten-Sicht und HA-Export filtern längst per `ist_dienstlich`; der Bericht zieht jetzt gleich. Betrifft nur Anlagen mit als *dienstlich* markiertem Fahrzeug (Demo-Anlage hat keins → dort keine Änderung).
  - **DI-4 — HA-Export-WP-Kosten bewerten den WP-Strom jetzt mit dem WP-Spezialtarif.** Der nach Home Assistant exportierte WP-Ersparnis-Sensor (und die WP-Ersparnis im Anlage-Aggregat) rechneten den WP-Stromverbrauch pauschal mit dem allgemeinen Netzbezugspreis — anders als der Monatsbericht, der den gepflegten WP-Tarif (§14a, oft günstiger) bevorzugt. Beide Stellen nutzen jetzt den SoT-Helper `resolve_strompreis_for_komponente` (WP-Tarif bevorzugt, Fallback allgemein). Betrifft nur Anlagen mit eigenem WP-Tarif (Demo-Anlage hat keinen → dort keine Änderung).
  - **DI-5 — Vorjahres-Vergleich des Gesamt-Nettoertrags war unvollständig.** Im T-Konto verglich das Vorjahres-Δ den Gesamt-Nettoertrag des aktuellen Monats (Einspeisung + Eigenverbrauch + **Wärmepumpe + E-Mobilität** − Netzbezug) mit einem Vorjahreswert, der **WP- und E-Mob-Ersparnis nicht enthielt** — ein methodischer Scheinsprung, sobald WP oder E-Auto existieren. Der Vorjahrespfad bildet die Kennzahl jetzt mit derselben Zusammensetzung (und respektiert dabei das Anschaffungsdatum je Komponente). Demo: Vorjahres-Nettoertrag Juni **253,32 → 324,02 €**, Januar **−13,38 → 72,97 €**. (Die orthogonale Frage, dass der aktuelle-Monat-Pfad für zurückliegende Monate heutige statt historische Tarife nutzt, bleibt davon unberührt.)
- **Drift-Inventur Folge-Paket (DI-2) — die drei beim DI-Bau aufgetauchten Folge-Abweichungen vor dem großen Release geschlossen.** Auch hier: Korrektur, kein Rechenfehler in der Kern-Bilanz — betroffene Werte ändern sich sichtbar:
  - **DI-2-A — CO₂-Ersparnis der Wärmepumpe im WP-Dashboard war zu niedrig.** Das Wärmepumpen-Dashboard war die vierte Stelle, die die WP-CO₂-Ersparnis selbst rechnete — mit dem korrekten Gas-Faktor, aber **ohne** den Gas-Kessel-Wirkungsgrad (die vermiedene Wärme muss erst über η_gas in Brennstoff zurückgerechnet werden). Nach DI-1 wich sie damit sichtbar von Cockpit/Bericht/Nachhaltigkeit ab. Jetzt nutzt auch das Dashboard den kanonischen Helper `co2_wp_ersparnis_kg`. Demo (Anlage 1, gesamt): **2303,6 → 2744,3 kg** — deckungsgleich mit der Summe der Cockpit-Jahreswerte. Ein zweiter statischer Wächter (Gas-CO₂-Faktor nur im Helper) + der um das Dashboard erweiterte Symmetrie-Test sichern alle **vier** Read-Sites ab.
  - **DI-2-B — HA-Export: Balkonkraftwerk und sonstige Erzeuger zählen jetzt zum Eigenverbrauch.** Die Eigenverbrauchs-Aggregation des HA-Exports zählte nur Dach-PV (`pv-module`) zur Erzeugung — Balkonkraftwerk und sonstige Erzeuger (z. B. Mini-BHKW hinter demselben Hauszähler) fehlten. Dadurch wichen die exportierten Sensoren **Eigenverbrauch, Autarkie, Eigenverbrauchsquote** und der daraus abgeleitete **CO₂-Sensor** bei solchen Anlagen vom Cockpit ab. Der HA-Export nutzt jetzt dieselbe Netzpunkt-Bilanz (`erzeugung_hinter_zaehler_kwh`) wie das Cockpit — Balkonkraftwerk zählt als PV, sonstige Erzeuger gehen in Eigenverbrauch/Autarkie ein (bleiben aber aus den PV-eigenen Kennzahlen und der CO₂-/Wirtschaftlichkeits-Bewertung eines Brennstoff-Erzeugers draußen). Demo (Anlage 1, gesamt): PV-Erzeugung **38.451,8 → 39.692,4 kWh**, Eigenverbrauch **14.465,4 → 17.366,0 kWh**, Autarkie **79,1 → 82,0 %**, EV-Quote **37,6 → 42,0 %**, CO₂ **12.800,1 → 13.902,3 kg** — jeweils exakt deckungsgleich mit dem Cockpit. Betrifft nur Anlagen mit Balkonkraftwerk oder sonstigem Erzeuger.
  - **DI-2-C — Vorjahres-Energieanzeige zeigte Werte vor dem Anschaffungsdatum.** DI-5 hatte den Vorjahres-**Finanz**-Pfad auf das Anschaffungsdatum je Komponente begrenzt; die Vorjahres-**Energieanzeige** (z. B. WP-Strom/-Wärme) zeigte dagegen weiter Monate, in denen die Komponente noch gar nicht in Betrieb war. Jetzt gilt die Anschaffungsdatum-Grenze auch dort. Demo (Vorjahres-Vergleich 2025→2024, WP angeschafft 04/2024): im Vorjahres-Januar entfallen die vor der Inbetriebnahme angezeigten **320 kWh WP-Strom / 1400 kWh WP-Wärme**; aktive Komponenten (PV, E-Mobilität) bleiben unverändert.

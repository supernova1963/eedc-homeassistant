"""
Energie-Profil-Service Backbone.

Schreib-Pfade leben in `services/energie_profil/` (siehe `__init__.py`):
...
Dieses Modul-File hält die geteilten Helper, die von den Slice-Files gebraucht
werden (Wetter-IST, SoC-History, Strompreis-Stunden, `_tage_zurueck`,
`StrompreisStunden`-Datenklasse). Lazy-Imports verhindern zirkuläre
Top-Level-Importe, da die Slice-Files hier ihrerseits einsetzen.

Die extrahierten Funktionen werden am Ende dieses Files re-exportiert,
damit bestehende Importer (`routes/energie_profil.py`,
`routes/monatsabschluss.py`, `services/scheduler.py`) unverändert bleiben.
"""
...
# ── Re-Exports der extrahierten Schreib-Pfade (Etappe 3c P3 + 3d P3) ────────
# Bestehende Importer (`routes/energie_profil.py`, `routes/monatsabschluss.py`,
# `services/scheduler.py`) bleiben unverändert.
from backend.services.energie_profil.aggregator import aggregate_day  # noqa: E402, F401
from backend.services.energie_profil.backfill import (  # noqa: E402, F401
    BackfillResult,
    BackfillStatus,
...
from backend.services.energie_profil.rollup import rollup_month  # noqa: E402, F401
from backend.services.energie_profil.scheduler_jobs import (  # noqa: E402, F401
    aggregate_today_all,

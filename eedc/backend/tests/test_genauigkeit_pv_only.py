...
async def test_batterie_nettoladen_zaehlt_nicht(db):
...
async def test_pv_und_bkw_werden_summiert(db):
...
async def test_nur_verbraucher_ist_null(db):
...
async def test_leeres_komponenten_kwh_keine_ist(db):

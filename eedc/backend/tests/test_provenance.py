...
async def test_initial_write_applied(db):
    """Initial-Write auf leere Provenance → immer applied."""
    md = await _make_md(db)

    result = await write_with_provenance(
        db, md, "netzbezug_kwh", 100.0,
    ...
    assert (await _audit_decisions(db)) == ["applied"]


async def test_higher_priority_overrides_existing(db):
    """Bestehender FALLBACK-Eintrag wird von MANUAL überschrieben (höhere Priorität)."""
    md = await _make_md(db)

    await write_with_provenance(
        db, md, "netzbezug_kwh", 80.0,
...
async def test_equal_priority_last_writer_wins(db):
...
async def test_lower_priority_rejected(db):
    ...
    result = await write_with_provenance(
        db, md, "netzbezug_kwh", 999.0,  # destruktiver Wert
        source="fallback:sensor_snapshot", writer="snapshot_aggregator",
    )
    await db.commit()
    ...
    assert (await _audit_decisions(db)) == ["applied", "rejected_lower_priority"]


async def test_no_op_same_value_with_input_hash(db):
    ...
    assert (await _audit_decisions(db)) == ["applied", "no_op_same_value"]


async def test_force_override_breaks_hierarchy(db):
...
async def test_manual_overrides_repair_unconditionally(db):
...
async def test_manual_subkey_overrides_repair_unconditionally(db):
    ...
    """
    imd = await _make_imd(db)

    await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "pv_erzeugung_kwh", 999.0,
    ...
    result = await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "pv_erzeugung_kwh", 0.0,
        source="manual:form", writer="alice@example.com",
    )
    await db.commit()
...
async def test_ha_lts_hourly_daily_labels_accepted(db):
    ...
    md1 = await _make_md(db)
    await write_with_provenance(
        db, md1, "netzbezug_kwh", 50.0,
        source="auto:monatsabschluss", writer="rollup_op",
    )
    result = await write_with_provenance(
        db, md1, "netzbezug_kwh", 75.0,
    ...
    md2 = await _make_md(db, jahr=2026, monat=6)
    await write_with_provenance(
        db, md2, "einspeisung_kwh", 100.0,
        source="fallback:sensor_snapshot", writer="snap_agg",
    )
    result = await write_with_provenance(
        db, md2, "einspeisung_kwh", 110.0,
    ...
    md3 = await _make_md(db, jahr=2026, monat=7)
    await write_with_provenance(
        db, md3, "netzbezug_kwh", 42.0,
        source="manual:form", writer="alice@example.com",
    )
    result = await write_with_provenance(
        db, md3, "netzbezug_kwh", 88.0,
        source="external:ha_statistics:daily", writer="lts_reader",
    )
    await db.commit()
    assert result.applied is False
...
async def test_unknown_source_raises_keyerror(db):
...
async def test_json_subkey_initial_write_applied(db):
    ...
    result = await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "km_gefahren", 1200.0,
        source="manual:form", writer="user@example.com",
    )
    await db.commit()
...
async def test_json_subkey_per_field_hierarchy_protection(db):
    ...
    r1 = await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "km_gefahren", 1500.0,  # destruktiv
        source="external:cloud_import:fronius_solarweb",
        writer="cloud_account_42",
    )
    r2 = await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "ladung_kwh", 130.0,
        source="external:cloud_import:fronius_solarweb",
        writer="cloud_account_42",
    )
    await db.commit()
    ...
    assert (await _audit_decisions(db)) == ["applied", "rejected_lower_priority", "applied"]


async def test_json_subkey_force_override_writes_repair(db):
    ...
    result = await write_json_subkey_with_provenance(
        db, imd, "verbrauch_daten", "km_gefahren", 800.0,
        source="manual:form", writer="repair_op_99",
        force_override=True,
    )
    await db.commit()

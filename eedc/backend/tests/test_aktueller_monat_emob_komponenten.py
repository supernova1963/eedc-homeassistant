...
Self-contained:
    ...
    eedc/backend/venv/bin/python -m pytest \
        eedc/backend/tests/test_aktueller_monat_emob_komponenten.py
"""
...
@pytest.mark.asyncio
async def test_aktueller_monat_eauto_komponente_nutzt_wb_pool_bei_evcc():
...
@pytest.mark.asyncio
async def test_aktueller_monat_premium_setup_unveraendert():
...
@pytest.mark.asyncio
async def test_aktueller_monat_multi_car_km_anteilig():
...
if __name__ == "__main__":
    ...
    failed = []
    sync_tests = [
        test_helper_evcc_setup_use_wb_pool_true,
        test_helper_premium_setup_use_wb_pool_false,
        test_helper_attribute_by_km_share_single_car,
        test_helper_attribute_by_km_share_multi_car,
        test_helper_pool_false_returns_zero_share,
    ]
    async_tests = [
        test_aktueller_monat_eauto_komponente_nutzt_wb_pool_bei_evcc,
        test_aktueller_monat_premium_setup_unveraendert,
        test_aktueller_monat_multi_car_km_anteilig,
    ]
    ...
    if failed:
        print(f"\n{len(failed)} Test(s) fehlgeschlagen: {failed}")
        sys.exit(1)
    print(f"\nAlle {len(sync_tests) + len(async_tests)} Tests erfolgreich.")

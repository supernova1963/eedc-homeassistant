...
async def test_ytd_mit_pvgis_wird_annualisiert(db):
    ...
    anlage, pv = await _setup_anlage_5kwp(db)
    _add_pvgis_prognose(db, anlage.id)
    ...
    for m in (1, 2, 3, 4):
        ...
        _add_imd(db, pv.id, 2026, m, kwh)
    await db.commit()

    resp = await get_cockpit_uebersicht(anlage_id=anlage.id, jahr=2026, db=db)
...
async def test_komplettes_jahr_unveraendert(db):
...
async def test_fallback_ohne_pvgis_nutzt_52n_verteilung(db):
...
async def test_alle_jahre_mit_anlagen_erweiterung(db):
    ...
    for jahr in range(2020, 2026):
        for m in range(1, 13):
            ...
            _add_imd(db, pv_a.id, jahr, m, kwh)
    # B: 2023–2025 (3 Jahre × 6000 kWh)
    for jahr in range(2023, 2026):
        for m in range(1, 13):
            kwh = 6000.0 * (SEASON_52N[m] / 100.0)
            _add_imd(db, pv_b.id, jahr, m, kwh)
    await db.commit()

    resp = await get_cockpit_uebersicht(anlage_id=anlage.id, jahr=None, db=db)
...
async def test_alle_jahre_ignoriert_vor_pv_monate(db):
...
async def test_alle_jahre_mit_teil_stilllegung(db):
    ...
    for jahr in range(2020, 2026):
        for m in range(1, 13):
            _add_imd(db, pv_a.id, jahr, m, 6000.0 * (SEASON_52N[m] / 100.0))
    # B: 2020–2022 (3 Jahre × 4000 kWh) — stillgelegt Ende 2022
    for jahr in range(2020, 2023):
        for m in range(1, 13):
            _add_imd(db, pv_b.id, jahr, m, 4000.0 * (SEASON_52N[m] / 100.0))
    await db.commit()

...
export function SonstigePositionenFields({ positionen, onChange }: SonstigePositionenFieldsProps) {
  ...
  const ertraege = positionen.filter(p => p.typ === 'ertrag').reduce((sum, p) => sum + (p.betrag || 0), 0)
  const ausgaben = positionen.filter(p => p.typ === 'ausgabe').reduce((sum, p) => sum + (p.betrag || 0), 0)
        ...
        <div className="space-y-2">
          ...
          {positionen.map((pos, index) => (
            <div key={index} className="grid grid-cols-12 gap-2 items-end">
              <div className="col-span-5">
                {index === 0 && <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Bezeichnung</label>}
                <input
                  type="text"
              ...
              <div className="col-span-3">
                {index === 0 && <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Betrag (EUR)</label>}
                <input
                  type="number"
                  step="0.01"
                  min="0"
              ...
              <div className="col-span-3">
                {index === 0 && <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Typ</label>}
                <select
                  value={pos.typ}
              ...
              <div className="col-span-1 flex justify-center">
                {index === 0 && <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">&nbsp;</label>}
                <button
                  type="button"
          ...
          {positionen.length > 0 && (
            <div className="text-xs flex gap-3 pt-1">
              <span className="text-green-600 dark:text-green-400">Ertr&auml;ge: {ertraege.toFixed(2)} &euro;</span>
              <span className="text-red-600 dark:text-red-400">Ausgaben: {ausgaben.toFixed(2)} &euro;</span>

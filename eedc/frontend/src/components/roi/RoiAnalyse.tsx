/**
 * RoiAnalyse — die reine ROI-Analyse (SoT), param-getrieben + ohne Seiten-Chrom.
 *
 * Ausgelagert aus `pages/ROIDashboard.tsx` (verhaltensgleich) → geteilt von der
 * IST-Seite UND der v4-Auswertung „ROI" (eine Code-Wahrheit). Enthält KPIs,
 * Amortisationskurve, ROI-nach-Typ, Investitions-Vergleich, Detail-Tabelle mit
 * Speicher-C-Detail (#264). Die Berechnungs-Parameter (Strompreis/Einspeise-
 * vergütung/Benzinpreis/Jahr) kommen als Props — die IST-Seite reicht ihre
 * Slider durch, die v4-Sicht die Anlagen-Defaults (D3: „rebuild-lite, Slider weg").
 * KEIN Anlagen-Selektor (global in der Shell), KEINE Parameter-Slider.
 */
...
import { Card, Alert, LoadingSpinner, EmptyState, FormelTooltip, fmtCalc, QuelleBadge, KPICard, ChartLegende } from '../ui'
...
export function RoiAnalyse({ anlageId, strompreis, einspeiseverguetung, benzinpreis, jahr, onLoaded }: RoiAnalyseProps) {
  ...
  if (roiData && roiData.berechnungen.length === 0) {
    return (
      <EmptyState
        icon={PiggyBank}
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={investitionenChart} layout="vertical">
              ...
              <Tooltip content={<ChartTooltip formatter={(value, name) => name === 'Relevante Kosten' ? `${value.toLocaleString('de-DE')} €` : `${value.toLocaleString('de-DE')} €/Jahr`} />} />
              <Legend content={<ChartLegende />} />
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">CO2</th>
              </tr>
            </thead>
                  ...
                  <Fragment key={b.investition_id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-right">
                        <p className="text-sm text-gray-900 dark:text-white">{b.relevante_kosten.toLocaleString('de-DE')} €</p>
                        {b.anschaffungskosten_alternativ > 0 && (
                          <p className="text-xs text-gray-500 dark:text-gray-400">({b.anschaffungskosten.toLocaleString('de-DE')} € gesamt)</p>
                        )}
                      </td>
                    ...
                    {cDetail && isExpanded && (
                      <tr>
                        <td colSpan={6} className="p-0"><SpeicherDetailPanel detail={cDetail} /></td>
                      </tr>
                    )}
                  </Fragment>
                )
            ...
            <tfoot>
              <tr className="bg-gray-50 dark:bg-gray-800 font-semibold">
                ...
                <td className="px-4 py-3 text-right text-sm text-gray-900 dark:text-white">{roiData.gesamt_relevante_kosten.toLocaleString('de-DE')} €</td>
                <td className="px-4 py-3 text-right text-sm text-green-600 dark:text-green-400">{roiData.gesamt_jahres_einsparung.toLocaleString('de-DE')} €</td>
                ...
                <td className="px-4 py-3 text-right text-sm text-emerald-600 dark:text-emerald-400">{roiData.gesamt_co2_einsparung_kg.toLocaleString('de-DE')} kg</td>
              </tr>
            </tfoot>
          </table>
        </div>
